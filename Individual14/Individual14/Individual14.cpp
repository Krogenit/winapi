...
int i = 1, n = 0;//i - текущее число в таймере, n - количество чисел в масиве
...
void CALLBACK timerProc(HWND hwnd, UINT uMsg, UINT timerId, DWORD dwTime)
{
	...
	if (summ > 100000)//если число больше int_max, то убиваем таймер
		KillTimer(hwnd, 0);

	arr[n].x = x;//присваиваем нашему масиву коориднаты
	arr[n].y = y;
	arr[n].chars = to_wstring(summ);//присваемвам число to_wstring переводит число в строку
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case ID_DOCH1:
			...
			break;
		case ID_DOCH2:
			SetTimer(hWnd, 0, 100, &timerProc);//при открытии окна запускается таймер с задержкой 8, можно менять, &timerProc - ссылка на нашу функцию таймера
			createWindow2(hWnd);
			break;
		default:
